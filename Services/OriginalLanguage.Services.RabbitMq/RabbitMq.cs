...
public class RabbitMq : IRabbitMq, IDisposable
{
    ...
    private async Task AddListener(string queueName,
        EventHandler<BasicDeliverEventArgs> onReceive, int? messageLifetime = null)
    {
        Connect();
    ...
    public async Task Subscribe<T>(string queueName, OnDataReceiveEvent<T> onReceive)
    {
        ...
        await AddListener(queueName, async (_, eventArgs) =>
        {
